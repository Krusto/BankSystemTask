...
auto BankSystem::CashIn(std::string_view name, CashT amount)
    -> BankSystem::OpResult {
  return HandleOperation({OperationType::CashIn, {amount, {}}, {name, {}}});
}

auto BankSystem::CashOut(std::string_view name, CashT amount)
    -> BankSystem::OpResult {
  return HandleOperation({OperationType::CashOut, {amount, {}}, {name, {}}});
}

auto BankSystem::Transfer(std::string_view from, std::string_view to,
                          CashT amount) -> BankSystem::OpResult {
...
auto BankSystem::HandleOperation(OpData operation) -> BankSystem::OpResult {
  ...
  if (auto result = CheckPreconditions(operation); result.has_value()) {
    users = result.value();
  } else {
...
auto BankSystem::FindUser(std::string_view name) const
    -> std::expected<UserID, OperationError> {
...
auto BankSystem::GetUserBalance(std::string_view name) const
    -> std::expected<CashT, OperationError> {
  if (auto result = FindUser(name); result.has_value()) {
    return m_UsersData.at(result.value()).balance;
  } else {
    return std::unexpected(result.error());
  }
}

auto BankSystem::GetUserStatus(std::string_view name) const
    -> std::expected<UserStatus, OperationError> {
  if (auto result = FindUser(name); result.has_value()) {
    return m_UsersData.at(result.value()).status;
  } else {
    return std::unexpected(result.error());
  }
}

auto BankSystem::CheckPreconditions(OpData &operation)
    -> std::expected<std::array<UserID, 2>, OperationError> {
  std::array<UserID, 2> users;
  if (auto result = FindUser(operation.users[0]); result.has_value()) {
    users[0] = result.value();
  } else {
    return std::unexpected(result.error());
  }
  ...
  switch (operation.type) {
  case OperationType::Transfer:
    if (auto result = FindUser(operation.users[1]); result.has_value()) {
      users[1] = result.value();
    } else {
      return std::unexpected(result.error());
    }
    break;
...
auto BankSystem::PrintData() -> void {
  ...
  for (const auto &[id, user] : m_Users) {
    ...
    LOG("User: {} Balance: {:.2f}{}", user.GetName().data(),
        data.balance.GetAmountAsDouble(),
