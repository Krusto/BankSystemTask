...
auto BankSystem::RegisterUser(std::string_view name) -> bool
{
    ...
    auto id = User::GetNextID();
    m_Users.emplace(std::piecewise_construct, std::forward_as_tuple(id), std::forward_as_tuple(name, id));
    m_UsersData.emplace(id, UserData{UserStatus::Active, {}});
    return true;
}
